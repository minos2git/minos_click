...
public: //added by xl for Minos
	int CID;
	int CW;
    Element(int cid);
	void setcid(int cid) {CID=cid;_inline_ports[0]._cid=cid; _inline_ports[1]._cid=cid;
		_inline_ports[2]._cid=cid;_inline_ports[3]._cid=cid;}

	void setcw(int w){CW = w; _inline_ports[0]._cw=w;  _inline_ports[1]._cw =w; 
		_inline_ports[2]._cw=w; _inline_ports[3]._cw=w;}

	static bool 	check_RAT(Packet *p) ;
	static void    release_packet(Packet *p);
