...
//added by xl for Minos
#define PACKET_TAILROOM   100
#define WAID_MAX_LEN   80

#define PACKET_MAKE_CID1_       40961       //40960 +1
#define PACKET_MAKE_CID2_      40962       //40960 +2
#define PACKET_MAKE_CID3_       40964       //40960 +4
#define PACKET_MAKE_CID4_       40968       //40960 +8
#define PACKET_CLONE_CID_       40976       //40960 +16
#define PACKET_UNIQUEIFY_CID_   40992       //40960 +32
#define PACKET_SHIFT_DATA_CID_  41024       //40960 +64
#define PACKET_DATA_CID_        41089       //40960 +128 + 1
#define PACKET_ENDDATA_CID_     41090       //40960 +128 + 2
#define PACKET_SHRINK_DATA_CID_ 41092       //40960 +128 + 4
#define PACKET_CHANGE_ROOM_CID_ 41096       //40960 +128 + 8
#define PACKET_KILL_CID_        41104       //40960 +128 + 16
...
public:
	...
	void attach_cid(int cid, int w); //added by xl
	void attach_cid(int cid, int w) const; //added by xl
	static bool check_repeat;


	static bool initial_tag ;
	static int PACKET_MAKE_CID1       ; 
	static int PACKET_MAKE_CID1_w       ; 
	static int PACKET_MAKE_CID2       ; 
	static int PACKET_MAKE_CID2_w       ; 
	static int PACKET_MAKE_CID3       ; 
	static int PACKET_MAKE_CID3_w       ; 
	static int PACKET_MAKE_CID4       ; 
	static int PACKET_MAKE_CID4_w       ; 
	static int PACKET_CLONE_CID       ; 
	static int PACKET_CLONE_CID_w       ; 
	static int PACKET_UNIQUEIFY_CID   ; 
	static int PACKET_UNIQUEIFY_CID_w   ; 
	static int PACKET_SHIFT_DATA_CID  ; 
	static int PACKET_SHIFT_DATA_CID_w  ; 
	static int PACKET_DATA_CID        ; 
	static int PACKET_DATA_CID_w        ; 
	static int PACKET_SHRINK_DATA_CID ; 
	static int PACKET_SHRINK_DATA_CID_w ; 
	static int PACKET_CHANGE_ROOM_CID ; 
	static int PACKET_CHANGE_ROOM_CID_w ; 
	static int PACKET_KILL_CID        ; 
	static int PACKET_KILL_CID_w        ; 
	
	static void initialize_comp_id(int &start_id, Vector<String> &comp_names, int comp_w[], HashTable<int, int> repeat);
	static inline void check_repeat_id(int &start_id, String c_name, Vector<String> &comp_names, HashTable<int, int> repeat);
...
inline void Packet::check_repeat_id(int &start_id, String c_name, Vector<String> &comp_names, HashTable<int, int> repeat)
{
	HashTable<int, int>::iterator it_rp = repeat.find(start_id);
	//std::map<int, int>::iterator it_rp = repeat.find(start_id); 
	if (it_rp != repeat.end())//exist
	{
		for (int rp_i=0; rp_i<it_rp.value(); rp_i++)
		{
			char rp_s[5] = {0};
			sprintf(rp_s, "-%d", rp_i);
			comp_names.push_back( c_name + rp_s ); //same names
			start_id++;
		}
	}
	else {//not exist 
		comp_names.push_back( c_name ); //same names
		start_id++;
	}
}
...
Packet::data() const
{
	attach_cid(PACKET_DATA_CID, PACKET_DATA_CID_w);
#if CLICK_LINUXMODULE
    return skb()->data;
#else
    return _data;
...
{
    return make(default_headroom, data, length, PACKET_TAILROOM );
}
...
{
    return make(default_headroom, (const unsigned char *) 0, length, PACKET_TAILROOM );
}
...
# endif
	Packet *p = reinterpret_cast<Packet *>(nskb);
	p->attach_cid(PACKET_MAKE_CID3, PACKET_MAKE_CID3_w);
    return p;//reinterpret_cast<Packet *>(nskb);
}
#endif
...
Packet::kill()
{
	//PACKET_KILL_CID is attached by element::destroy_packet
#if CLICK_LINUXMODULE
...
Packet::make(struct mbuf *m)
{
  ...
  p->attach_cid(PACKET_MAKE_CID4 , PACKET_MAKE_CID4_w);
  return p;
}
#endif
...
Packet::uniqueify()
{
	WritablePacket *p;
    if (!shared())
		p = static_cast<WritablePacket *>(this);
    else
		p = expensive_uniqueify(0, 0, true);
	
	p->attach_cid( PACKET_UNIQUEIFY_CID, PACKET_UNIQUEIFY_CID_w  );
	return p;
}
...
Packet::shrink_data(const unsigned char *data, uint32_t length)
{
	...
	//added by xl
	_end = _tail + WAID_MAX_LEN;
	attach_cid( PACKET_SHRINK_DATA_CID, PACKET_SHRINK_DATA_CID_w  );
    }
}
...
Packet::change_headroom_and_length(uint32_t headroom, uint32_t length)
{
	...
	attach_cid( PACKET_CHANGE_ROOM_CID, PACKET_CHANGE_ROOM_CID_w  );
}
#endif
